<?php
/**
 * Property Test: ADV Approval Status
 * 
 * **Feature: installation-module, Property 23: ADV approval status transition**
 * **Validates: Requirements 15.3**
 * 
 * Property: For any installation where ADV approves all sections, the overall
 * installation status should be 'adv_approved'.
 */
...
class AdvApprovalStatusPropertyTest {
    private $testResults = [];
    ...
    public function runTests(): bool {
        echo "\n=== ADV Approval Status Property Tests ===\n";
        echo "**Feature: installation-module, Property 23: ADV approval status transition**\n";
        echo "**Validates: Requirements 15.3**\n\n";
        
        // Property tests
        $this->runPropertyTest(
            'ADV approving all sections individually triggers adv_approved status',
            [$this, 'testAdvApprovingAllSectionsIndividuallyTriggersAdvApproved']
        );
        
        $this->runPropertyTest(
            'Using approveAllSections with ADV level triggers adv_approved status',
            [$this, 'testApproveAllSectionsAdvLevelTriggersAdvApproved']
        );
        
        $this->runPropertyTest(
            'ADV partial approval does not trigger adv_approved status',
            [$this, 'testAdvPartialApprovalDoesNotTriggerAdvApproved']
        );
        
        $this->runPropertyTest(
            'areAllSectionsApproved returns true for ADV when all sections approved',
            [$this, 'testAreAllSectionsApprovedReturnsTrueForAdv']
        );
    ...
    /**
     * Property Test: ADV approving all sections individually triggers adv_approved status
     * For any contractor-approved installation, when ADV approves all sections one by one,
     * status becomes adv_approved
     */
    private function testAdvApprovingAllSectionsIndividuallyTriggersAdvApproved(): array {
        ...
        // ADV approves all sections one by one
        foreach ($sections as $section) {
            ...
            if (!$result['success']) {
                return [
                    'success' => false,
                    'message' => "Failed to ADV approve section $section: " . $result['message']
                ];
            }
        }
        
        // Verify installation status is adv_approved
        $installation = $this->installationRepository->findById($installationId);
        if (!$installation) {
            return [
                'success' => false,
                'message' => 'Installation not found after ADV approvals'
            ];
        }
        ...
        if ($installation['status'] !== Installation::STATUS_ADV_APPROVED) {
            return [
                'success' => false,
                'message' => "Expected status 'adv_approved', got '{$installation['status']}'"
            ];
        }
    ...
    /**
     * Property Test: Using approveAllSections with ADV level triggers adv_approved status
     * For any contractor-approved installation, using bulk ADV approve sets status to adv_approved
     */
    private function testApproveAllSectionsAdvLevelTriggersAdvApproved(): array {
        // Create test installation in contractor_approved status
        $testData = $this->createTestInstallationInContractorApprovedStatus();
        if (!$testData['success']) {
            return $testData;
        ...
        if (!$result['success']) {
            return [
                'success' => false,
                'message' => 'Failed to ADV approve all sections: ' . $result['message']
            ];
        }
        ...
        if (!$installation) {
            return [
                'success' => false,
                'message' => 'Installation not found after ADV bulk approval'
            ];
        }
        ...
        // Verify the result data also indicates adv_approved
        if ($result['data']['installation_status'] !== Installation::STATUS_ADV_APPROVED) {
            return [
                'success' => false,
                'message' => "Result data expected 'adv_approved', got '{$result['data']['installation_status']}'"
            ];
        }
    ...
    /**
     * Property Test: ADV partial approval does not trigger adv_approved status
     * For any contractor-approved installation with only some ADV approvals, status should not be adv_approved
     */
    private function testAdvPartialApprovalDoesNotTriggerAdvApproved(): array {
        // Create test installation in contractor_approved status
        $testData = $this->createTestInstallationInContractorApprovedStatus();
        if (!$testData['success']) {
            return $testData;
        ...
        // ADV approves only half of the sections (at least 1, at most all-1)
        ...
        // Verify installation status is NOT adv_approved
        $installation = $this->installationRepository->findById($installationId);
        if (!$installation) {
            return [
                'success' => false,
                'message' => 'Installation not found after partial ADV approvals'
            ];
        }
        ...
        if ($installation['status'] === Installation::STATUS_ADV_APPROVED) {
            return [
                'success' => false,
                'message' => "Status should not be 'adv_approved' with only $sectionsToApprove of " . count($sections) . " sections ADV approved"
            ];
        }
    ...
    /**
     * Property Test: areAllSectionsApproved returns true for ADV when all sections approved
     * For any installation with all ADV approvals, areAllSectionsApproved should return true
     */
    private function testAreAllSectionsApprovedReturnsTrueForAdv(): array {
        // Create test installation in contractor_approved status
        $testData = $this->createTestInstallationInContractorApprovedStatus();
        if (!$testData['success']) {
            return $testData;
        ...
        if ($initialCheck === true) {
            return [
                'success' => false,
                'message' => 'areAllSectionsApproved for ADV should be false initially'
            ];
        }
        
        // ADV approves all sections
        $result = $this->reviewService->approveAllSections(
            $installationId,
            $reviewerId,
            null,
        ...
        if ($finalCheck !== true) {
            return [
                'success' => false,
                'message' => 'areAllSectionsApproved for ADV should be true after approving all sections'
            ];
        }
