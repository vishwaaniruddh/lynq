<?php
/**
 * Engineer Dispatch Page
 * 
 * Allows engineers to dispatch materials back to their contractor or return to ADV
 * Includes:
 * - Destination selector showing contractor and ADV
 * - Product/quantity selector from engineer's inventory
        ...
        <div class="p-6 border-b">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-sm text-gray-500">Return materials to your contractor or ADV warehouse</p>
                </div>
                <a href="pending-receives.php" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition">
                    <i class="fas fa-inbox mr-2"></i>Pending Receives
                </a>
            </div>
        </div>
    </div>
        ...
        <form id="dispatch-form" class="p-6 space-y-6">
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <label class="destination-option relative flex items-start p-4 border-2 rounded-xl cursor-pointer hover:border-primary/50 transition" data-type="contractor">
                        <input type="radio" name="destination_type" value="contractor" class="sr-only">
                        <div class="flex items-center">
                            <div class="w-12 h-12 bg-blue-100 rounded-lg flex items-center justify-center mr-4">
                                <i class="fas fa-building text-blue-500 text-xl"></i>
                            </div>
                            <div>
                                <p class="font-medium text-gray-800">Return to Contractor</p>
                                <p class="text-sm text-gray-500">Send materials back to your contractor</p>
                            </div>
                        </div>
                ...
                <!-- Contractor Selection (shown when returning to contractor) -->
                <div id="contractor-selection" class="hidden mt-4">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Select Contractor <span class="text-red-500">*</span></label>
                    <select id="contractor-select" class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full relative z-10 text-center p-8">
            ...
            <div class="flex justify-center gap-3">
                ...
                <a href="pending-receives.php" class="px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary/90 transition">
                    View Pending Receives
                </a>
            </div>
        </div>
    </div>
...
<!-- Pass user ID from PHP to JavaScript -->
<script>
const USER_ID = <?php echo json_encode($currentUser['id']); ?>;
...
const state = {
    ...
    inventory: [],
    selectedItems: [],
    destinations: { companies: [], warehouses: [] },
    userId: USER_ID,
...
function setupEventListeners() {
    ...
    document.getElementById('contractor-select').addEventListener('change', function() {
        state.selectedContractorId = this.value ? parseInt(this.value) : null;
        updateSubmitButton();
        updateSummary();
    });
...
function selectDestinationType(type) {
    ...
    document.getElementById('contractor-selection').classList.toggle('hidden', type !== 'contractor');
...
async function loadDestinations() {
    try {
        // Load valid destinations for engineer (user-level)
        const response = await fetch(`../../api/inventory/dispatch/destinations.php?sender_type=user&sender_id=${state.userId}`, {
            credentials: 'include'
        });
...
function populateDestinations() {
    // Populate contractors dropdown (engineer's assigned contractor)
    ...
    if (state.destinations.companies && state.destinations.companies.length > 0) {
        state.destinations.companies.forEach(company => {
            contractorSelect.innerHTML += `<option value="${company.id}">${company.name}</option>`;
        });
    } else {
        contractorSelect.innerHTML = '<option value="">No contractors available</option>';
    }
...
async function loadInventory() {
    ...
    try {
        // For engineer, inventory is at user level
        const response = await fetch(`../../api/inventory/counter/get.php?entity_type=user&entity_id=${state.userId}`, {
            credentials: 'include'
        });
        const result = await response.json();
...
function updateSubmitButton() {
    ...
    const hasDestination = (state.destinationType === 'contractor' && state.selectedContractorId) ||
...
function updateSummary() {
    ...
    if (state.destinationType === 'contractor' && state.selectedContractorId) {
        const contractor = state.destinations.companies.find(c => c.id == state.selectedContractorId);
        if (contractor) {
            parts.push(`To: ${contractor.name}`);
        }
...
function showConfirmModal() {
    ...
    let destinationHtml = '';
    if (state.destinationType === 'contractor') {
        const contractor = state.destinations.companies.find(c => c.id == state.selectedContractorId);
        destinationHtml = `
            <div class="flex items-center p-3 bg-gray-50 rounded-lg">
                ...
                <div>
                    <p class="text-sm text-gray-500">Returning to Contractor</p>
                    <p class="font-medium text-gray-800">${contractor ? contractor.name : 'Unknown'}</p>
                </div>
            </div>
        `;
    } else {
    ...
    detailsEl.innerHTML = destinationHtml + '<div class="mt-3"><p class="text-sm font-medium text-gray-700 mb-2">Items:</p>' + itemsHtml + '</div>';
...
async function submitDispatch() {
    ...
    try {
        // Build dispatch data - engineer dispatches from user level
        const dispatchData = {
            sender_type: 'user',
        ...
        if (state.destinationType === 'contractor') {
            dispatchData.to_company_id = state.selectedContractorId;
        } else {
...
function resetForm() {
    ...
    document.getElementById('contractor-selection').classList.add('hidden');
