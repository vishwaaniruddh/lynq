 ...
 * Provides data access for product categories
 * 
...
class ProductCategoryRepository extends BaseRepository {
    ...
    protected $companyIdColumn = null; // Categories are not company-specific
    ...
    public function findChildren($parentId) {
        $sql = "SELECT * FROM `{$this->table}` 
                ...
                ORDER BY `name`";
        return $this->db->getResults($sql, [$parentId, self::STATUS_ACTIVE], 'is');
    }
    ...
    public function canDelete($id) {
        return !$this->hasChildren($id) && $this->getProductCount($id) == 0;
    }
}
