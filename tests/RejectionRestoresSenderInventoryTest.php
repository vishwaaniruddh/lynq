<?php
/**
 * Property Test: Rejection Restores Sender Inventory
 * 
 * **Feature: inventory-dispatch-receive-flow, Property 5: Rejection Restores Sender Inventory**
 * **Validates: Requirements 4.1, 4.2, 7.6**
 * 
 * Property: For any pending receive that is rejected, the sender's inventory counter 
 * SHALL be restored by exactly the originally dispatched quantity.
 */
...
class RejectionRestoresSenderInventoryTest extends PropertyTestBase {
    ...
    public function runTests() {
        echo "=== Rejection Restores Sender Inventory Property Test ===\n";
        echo "**Feature: inventory-dispatch-receive-flow, Property 5: Rejection Restores Sender Inventory**\n";
        echo "**Validates: Requirements 4.1, 4.2, 7.6**\n\n";
        ...
        // Run property test for warehouse to company rejection
        $allPassed &= $this->runPropertyTest(
            'Property 5: Rejection restores sender inventory (warehouse to company)',
            function() {
                return $this->testRejectionRestoresWarehouseInventory();
            },
            100
        );
        
        // Run property test for company to user rejection
        $allPassed &= $this->runPropertyTest(
            'Property 5: Rejection restores sender inventory (company to user)',
            function() {
                return $this->testRejectionRestoresCompanyInventory();
            },
            100
        );
    ...
    /**
     * Test rejection restores warehouse inventory
     * 
     * Property: For any dispatch from warehouse to company that is rejected,
     * the warehouse's inventory counter should be restored by the dispatched quantity.
     */
    private function testRejectionRestoresWarehouseInventory() {
        ...
        $senderInventoryBefore = $this->inventoryCounterService->getCounter('warehouse', $warehouse['id'], $product['id']);
        
        // Create dispatch and pending receive
        ...
        $senderInventoryAfterDispatch = $this->inventoryCounterService->getCounter('warehouse', $warehouse['id'], $product['id']);
        
        // Verify dispatch decremented sender inventory
        $expectedAfterDispatch = $senderInventoryBefore - $dispatchQuantity;
        if ($senderInventoryAfterDispatch !== $expectedAfterDispatch) {
            return [
                'success' => false,
                'message' => "Dispatch did not decrement sender inventory correctly",
                'data' => [
                    'before' => $senderInventoryBefore,
                    'after_dispatch' => $senderInventoryAfterDispatch,
        ...
        $senderInventoryAfterRejection = $this->inventoryCounterService->getCounter('warehouse', $warehouse['id'], $product['id']);
        
        // Property check: sender inventory should be restored to original value
        if ($senderInventoryAfterRejection !== $senderInventoryBefore) {
            return [
                'success' => false,
                ...
                'data' => [
                    ...
                    'dispatch_quantity' => $dispatchQuantity,
                    'sender_before' => $senderInventoryBefore,
                    'sender_after_dispatch' => $senderInventoryAfterDispatch,
                    'sender_after_rejection' => $senderInventoryAfterRejection,
                    'expected_after_rejection' => $senderInventoryBefore
                ]
            ];
        }
    ...
    /**
     * Test rejection restores company inventory
     * 
     * Property: For any dispatch from company to user that is rejected,
     * the company's inventory counter should be restored by the dispatched quantity.
     */
    private function testRejectionRestoresCompanyInventory() {
        // Generate random test data
        $initialQuantity = $this->generateRandomInt(30, 150);
        $dispatchQuantity = $this->generateRandomInt(1, min($initialQuantity, 30));
        $rejectionReason = 'Test rejection reason: ' . $this->generateRandomString(15);
        ...
        // Create test company (sender)
        $company = $this->createTestCompany();
        if (!$company) {
            return ['success' => false, 'message' => 'Failed to create test company'];
        }
        
        // Create test warehouse for the company (required for dispatch)
        $warehouse = $this->createTestWarehouseForCompany($company['id']);
        if (!$warehouse) {
            return ['success' => false, 'message' => 'Failed to create test warehouse'];
        }
        
        // Create test users
        $senderUser = $this->createTestUser($company['id']);
        $recipientUser = $this->createTestUser($company['id']);
        if (!$senderUser || !$recipientUser) {
            return ['success' => false, 'message' => 'Failed to create test users'];
        }
        
        // Set initial inventory for company
        $incrementResult = $this->inventoryCounterService->incrementCounter(
            'company',
            $company['id'],
            $product['id'],
        ...
        // Get sender inventory before dispatch
        $senderInventoryBefore = $this->inventoryCounterService->getCounter('company', $company['id'], $product['id']);
        ...
        $senderInventoryAfterDispatch = $this->inventoryCounterService->getCounter('company', $company['id'], $product['id']);
        ...
        // Reject the pending receive
        $rejectResult = $this->receiveService->rejectReceive($pendingReceiveId, $recipientUser['id'], $rejectionReason);
        
        if (!$rejectResult['success']) {
            return ['success' => false, 'message' => 'Failed to reject pending receive: ' . $rejectResult['message']];
        }
        
        // Get sender inventory after rejection
        $senderInventoryAfterRejection = $this->inventoryCounterService->getCounter('company', $company['id'], $product['id']);
     ...
     */
    private function createTestWarehouseForCompany($companyId) {
        try {
            $warehouseData = [
                'name' => 'Test Warehouse ' . $this->generateRandomString(8),
                'location' => 'Test Location',
            ...
            error_log("Failed to create test warehouse for company: " . $e->getMessage());
            return null;
        }
    }
