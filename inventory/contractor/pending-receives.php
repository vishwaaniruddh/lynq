<?php
/**
 * Contractor Pending Receives Page
 * 
 * Displays all pending receives for contractor's company
 * Includes:
 ...
 * 
 * Requirements: 2.1, 2.3, 2.4, 2.5, 3.1, 4.1, 10.1, 10.5
 */
...
$currentPage = 'contractor_pending_receives';
$isLoggedIn = true;
$breadcrumbs = [
...
<div class="bg-white rounded-xl shadow-sm">
    ...
    <div class="overflow-x-auto overflow-y-visible">
        <table id="pending-receives-table" class="w-full text-sm">
            <thead class="bg-gray-50/80">
                <tr>
                    <th class="px-4 py-2.5 text-left">
                        <input type="checkbox" id="select-all" class="w-3.5 h-3.5 rounded border-gray-300" onchange="toggleSelectAll()">
                    </th>
                    <th class="px-4 py-2.5 text-left text-[11px] font-semibold text-gray-500 uppercase tracking-wider">Dispatch #</th>
                    <th class="px-4 py-2.5 text-left text-[11px] font-semibold text-gray-500 uppercase tracking-wider">Sender</th>
                    <th class="px-4 py-2.5 text-left text-[11px] font-semibold text-gray-500 uppercase tracking-wider">Items</th>
                    <th class="px-4 py-2.5 text-left text-[11px] font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">Dispatch Date</th>
                    <th class="px-4 py-2.5 text-left text-[11px] font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">Days Pending</th>
            ...
            <tbody id="pending-receives-tbody" class="divide-y divide-gray-100">
                <tr>
                ...
                <div class="border rounded-lg overflow-hidden">
                    <table class="w-full">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-3 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase">#</th>
                                <th class="px-3 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase">Product</th>
                                <th class="px-3 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase">Category</th>
                                <th class="px-3 py-2 text-center text-[10px] font-semibold text-gray-500 uppercase">Expected</th>
                                <th class="px-3 py-2 text-center text-[10px] font-semibold text-gray-500 uppercase">Received</th>
                                <th class="px-3 py-2 text-center text-[10px] font-semibold text-gray-500 uppercase">Discrepancy</th>
                                <th class="px-3 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase">Notes</th>
                            </tr>
                        </thead>
...
async function loadPendingReceives() {
    ...
    try {
        // Build query params
        let url = '../../api/inventory/receive/pending.php?view=company';
...
function renderTable() {
        ...
        tbody.innerHTML = `
            <tr>
                <td colspan="8" class="px-4 py-10 text-center text-gray-400">
                    ...
                    <p class="text-sm">No pending receives found</p>
                </td>
            </tr>
        `;
        return;
    ...
    tbody.innerHTML = state.pendingReceives.map((receive, index) => {
        ...
        const rowClass = isOverdue ? 'bg-red-50/50' : '';
            ...
            <tr class="${rowClass} hover:bg-gray-50/50 transition-colors">
                <td class="px-4 py-2.5">
                    ${receive.status === 'pending' ? `
                        <input type="checkbox" class="row-checkbox w-3.5 h-3.5 rounded border-gray-300" 
                            data-id="${receive.id}" onchange="toggleRowSelection(${receive.id})">
                    ` : '<span class="text-gray-300 text-xs">-</span>'}
                </td>
                <td class="px-4 py-2.5">
                    <span class="font-medium text-gray-800 text-xs">${receive.dispatch_number || 'N/A'}</span>
                </td>
                <td class="px-4 py-2.5">
                    <div>
                        <p class="font-medium text-gray-800 text-xs">${receive.sender_name || receive.sender_company_name || 'ADV'}</p>
                        <p class="text-[10px] text-gray-400">${receive.sender_type || 'warehouse'}</p>
                    </div>
                </td>
                <td class="px-4 py-2.5">
                    <span class="inline-flex items-center px-2 py-0.5 bg-blue-50 text-blue-600 rounded text-[10px] font-medium">
                        ${itemCount} item${itemCount !== 1 ? 's' : ''} (${totalQty} qty)
                    </span>
                </td>
                <td class="px-4 py-2.5 whitespace-nowrap">
                    <span class="text-xs text-gray-600">${formatDate(receive.dispatch_date || receive.created_at)}</span>
                </td>
                <td class="px-4 py-2.5 whitespace-nowrap">
                    ${receive.status === 'pending' ? `
                ...
                <td class="px-4 py-2.5">
                    <div class="flex items-center justify-center gap-0.5">
                        <button onclick="viewDetails(${receive.id})" class="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded transition-colors" title="View Details">
                            <i class="fas fa-eye text-xs"></i>
                        </button>
                        ${receive.status === 'pending' ? `
                            <button onclick="openAcceptModal(${receive.id})" class="p-1.5 text-gray-400 hover:text-green-600 hover:bg-green-50 rounded transition-colors" title="Accept">
                                <i class="fas fa-check text-xs"></i>
                            </button>
                            <button onclick="openPartialAcceptModal(${receive.id})" class="p-1.5 text-gray-400 hover:text-amber-600 hover:bg-amber-50 rounded transition-colors" title="Partial Accept">
                                <i class="fas fa-check-double text-xs"></i>
                            </button>
                            <button onclick="openRejectModal(${receive.id})" class="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded transition-colors" title="Reject">
...
function getStatusBadge(status, isOverdue = false) {
    const badges = {
        'pending': isOverdue 
            ? '<span class="inline-flex items-center px-2 py-0.5 bg-red-50 text-red-600 rounded-full text-[10px] font-medium"><span class="w-1.5 h-1.5 bg-red-400 rounded-full mr-1"></span>Overdue</span>'
            : '<span class="inline-flex items-center px-2 py-0.5 bg-amber-50 text-amber-600 rounded-full text-[10px] font-medium"><span class="w-1.5 h-1.5 bg-amber-400 rounded-full mr-1"></span>Pending</span>',
        ...
        'partial': '<span class="inline-flex items-center px-2 py-0.5 bg-amber-50 text-amber-600 rounded-full text-[10px] font-medium"><span class="w-1.5 h-1.5 bg-amber-400 rounded-full mr-1"></span>Partial</span>'
    };
    return badges[status] || '<span class="inline-flex items-center px-2 py-0.5 bg-gray-100 text-gray-500 rounded-full text-[10px]"><span class="w-1.5 h-1.5 bg-gray-400 rounded-full mr-1"></span>Unknown</span>';
}
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-3 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase">#</th>
                        <th class="px-3 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase">Product</th>
                        <th class="px-3 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase">Serial No.</th>
                        <th class="px-3 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase">Category</th>
                        <th class="px-3 py-2 text-right text-[10px] font-semibold text-gray-500 uppercase">Qty</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    ${items.map((item, index) => `
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-3 py-2 text-xs font-medium text-gray-800">${item.product_name || 'Unknown Product'}</td>
                            <td class="px-3 py-2 text-xs text-gray-500 font-mono">${item.serial_number || '-'}</td>
                            <td class="px-3 py-2"><span class="px-2 py-0.5 bg-purple-50 text-purple-600 rounded text-[10px]">${item.category_name || item.product_category || '-'}</span></td>
                            <td class="px-3 py-2 text-xs font-medium text-gray-800 text-right">${item.expected_quantity || item.quantity || 1}</td>
                        </tr>
                    `).join('')}
                </tbody>
            </table>
        </div>
        <div class="mt-3 pt-3 border-t flex justify-between font-medium text-sm">
...
function openPartialAcceptModal(id) {
    ...
    tbody.innerHTML = items.map((item, index) => `
        <tr class="hover:bg-gray-50">
            ...
            <td class="px-3 py-2">
                <div>
                    <p class="font-medium text-gray-800 text-xs">${item.product_name || 'Unknown Product'}</p>
                    ${item.serial_number ? `<p class="text-[10px] text-gray-400">S/N: ${item.serial_number}</p>` : ''}
                </div>
                <input type="hidden" class="item-dispatch-id" value="${item.dispatch_item_id || item.id}">
            </td>
            <td class="px-3 py-2">
                <span class="px-2 py-0.5 bg-purple-50 text-purple-600 rounded text-[10px]">${item.category_name || item.product_category || '-'}</span>
            </td>
            <td class="px-3 py-2 text-center">
                <span class="font-medium text-gray-800 text-xs expected-qty">${item.expected_quantity || item.quantity || 1}</span>
            </td>
            <td class="px-3 py-2 text-center">
                <input type="number" class="received-qty w-16 px-2 py-1 border rounded text-center text-xs focus:ring-2 focus:ring-primary" 
            ...
            <td class="px-3 py-2 text-center">
                <span class="discrepancy-display text-xs text-gray-500">0</span>
            </td>
            <td class="px-3 py-2">
                <input type="text" class="item-notes w-full px-2 py-1 border rounded text-xs focus:ring-2 focus:ring-primary" 
                    placeholder="Notes...">
            </td>
        </tr>
    `).join('');
